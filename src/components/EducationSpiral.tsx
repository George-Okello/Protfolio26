...
import { GraduationCap, BookOpen, Orbit, ArrowRight, ShieldCheck } from "lucide-react";
    ...
    <div className="relative py-24" id="academic-education" ref={containerRef}>
      ...
      <div className="relative z-10">
        <div className="text-center mb-16 md:mb-24">
          ...
          <h3 className="text-3xl md:text-5xl font-serif tracking-tight mb-4">Educational Trajectory</h3>
          <p className="text-xs font-mono uppercase tracking-widest opacity-60">Degrees & Specialized Certifications</p>
        </div>
        ...
        <div className="relative max-w-5xl mx-auto px-4 md:px-0 flex flex-col items-center mb-24">
          {educations.map((edu, idx) => {
            ...
            return (
              <motion.div 
                key={edu.id}
                initial={{ opacity: 0, x: offsetX, y: 100, rotateZ: idx % 2 === 0 ? -3 : 3 }}
              ...
              >
                <div className="flex flex-col md:flex-row md:items-start gap-6">
                  ...
                  <div className={`p-4 rounded-xl shrink-0 ${isDark ? "bg-slate-900 border border-slate-800" : "bg-slate-50 border border-slate-200"}`}>
                    <GraduationCap className="w-8 h-8 text-orange-500" />
                  </div>

                  <div className="flex-1">
                    <h4 className="text-2xl font-serif leading-tight mb-2">{edu.degree}</h4>
                    <div className="flex items-center gap-2 mb-6">
                      <span className="text-xs font-mono uppercase tracking-widest opacity-70">{edu.school}</span>
                      <span className="opacity-30">•</span>
                      <span className="text-xs font-mono opacity-50">{edu.location}</span>
                    </div>
                    ...
                    {edu.thesis && (
                      <div className={`p-5 border-l-2 border-orange-500 ${isDark ? "bg-orange-500/5" : "bg-orange-50"}`}>
                        ...
                        <div className="font-medium text-sm mb-1">{edu.thesis.title}</div>
                        <div className="text-xs opacity-70 leading-relaxed">{edu.thesis.description}</div>
                      </div>
                    )}
                  </div>
        ...
        <div className="max-w-6xl mx-auto px-6 md:px-12">
          <div className="mb-12 text-center">
            <h4 className="text-2xl font-serif tracking-tight">Specialized Certifications</h4>
          </div>
            ...
            {certifications.map((cert, idx) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs leading-relaxed opacity-60">{cert.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
