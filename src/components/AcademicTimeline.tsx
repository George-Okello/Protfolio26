...
export default function AcademicTimeline({ theme }: AcademicTimelineProps) {
  ...
  const renderCard = (
    ...
    const spanClass =
      (totalInYear % 2 !== 0 && index === totalInYear - 1) || config.isCurrent
        ? "md:col-span-2"
        : "col-span-1";
        ...
        >
          ...
          <div className="absolute top-0 right-0 p-6 pointer-events-none overflow-hidden opacity-60 group-hover:opacity-100 group-active:opacity-100 transition-opacity duration-500">
            <div className="relative flex items-center justify-center w-24 h-24">
              ...
              />
              <div
                className={`absolute inset-2 rounded-full border ${isDark ? "border-amber-500/20" : "border-amber-300/60"} animate-[spin_8s_linear_infinite_reverse]`}
              />
              <div
                className={`absolute inset-4 rounded-full flex items-center justify-center ${isDark ? "bg-amber-500/10" : "bg-amber-100/60"}`}
              >
                <Award
          ...
          <div className="flex flex-col relative z-10 h-full">
            <div className="flex items-center gap-3 mb-6">
              <div
                className={`p-2 rounded flex items-center justify-center ${isDark ? "bg-amber-500/10 border border-amber-500/20" : "bg-amber-100/50 border border-amber-200"}`}
              >
                <Award
              ...
              <div
                className={`text-[10px] font-mono uppercase tracking-widest font-semibold ${isDark ? "text-amber-400/80" : "text-amber-600/80"}`}
              >
                {item.period} • Award
              </div>
            </div>
            ...
            <p
              className={`text-[10px] uppercase tracking-widest mb-5 font-mono ${isDark ? "text-slate-400" : "text-slate-500"}`}
            >
              {item.issuer}
            </p>
            <p
              className={`text-sm font-light leading-relaxed mb-8 flex-1 w-5/6 ${isDark ? "text-slate-300" : "text-slate-600"}`}
            >
              {item.description}
            </p>
            {item.link && (
      ...
      >
        <div className="flex flex-col mb-6">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span
              className={`px-3 py-1.5 rounded-full text-[10px] font-mono font-medium ${
                isDark
